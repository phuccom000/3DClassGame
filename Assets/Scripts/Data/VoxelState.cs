...
public class VoxelState
{
    ...
    public byte light
    {
        ...
        set
        {
            if (value != _light)
            {
                ...
                // If our new light value is darker than the old one, check our neighboring voxels.
                if (_light < oldLightValue)
                {
                    ...
                    for (int p = 0; p < 6; p++)
                    {
                        ...
                        if (neighbours[p] != null)
                        {
                            ...
                            // is being lit from somewhere else. We then tell that voxel to propagate and, if it
                            // is lighter than this voxel, light will be propagated to here.
                            else
                            {
    ...
    public void PropagateLight()
    {
        ...
        for (int p = 0; p < 6; p++)
        {
            // We can only propagate to voxels that exist so check there is one first.
            if (neighbours[p] != null)
            {
                // We can ONLY propagate light in one direction (lighter to darker). If
                // we work in both direction, we will get recursive loops.
